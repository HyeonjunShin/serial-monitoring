...
constexpr int MCU_CLOCK = 53760000;
constexpr int BAUD_RATE = 921600;
constexpr std::string_view SERIAL_PORT = "/dev/ttyUSB0";
...
class DeviceV1 : public Device {
public:
  DeviceV1() : Device() {}
  const uint16_t calcCRC(const uint8_t *data, size_t length) override {
    uint16_t crc = 0;
  ...
  const uint8_t *getHeader() const override { return HEADER; }
  const int getPacketSize() const override { return PACKET_SIZE; }
  const uint16_t *getCRCTable() const override { return CRC_TABLE; }
  const std::string_view getSerialPort() const override { return SERIAL_PORT; }
  const int getBaudRate() const override { return BAUD_RATE; }
};
