...
class RealtimeSpectrogramWidget : public QWidget {
  Q_OBJECT
public:
  explicit RealtimeSpectrogramWidget(QWidget *parent = nullptr)
...
private:
  std::deque<std::vector<double>> dataBuffer;
  std::mutex dataMutex;
  QTimer *updateTimer;
  size_t maxHistory;
  int freqBins;
  QVector<QColor> turbo;

  void initializeTurboColormap() {
    turbo.resize(256);
    // Google Turbo colormap lookup table
    static const double turbo_data[256][3] = {
    ...
    for (int i = 0; i < 256; ++i) {
      turbo[i] = QColor::fromRgbF(turbo_data[i][0], turbo_data[i][1],
                                  turbo_data[i][2]);
    }
  }

  QColor turboColor(double v) const {
    int idx = std::clamp(static_cast<int>(v * 255.0), 0, 255);
    return turbo[idx];
  }
};
